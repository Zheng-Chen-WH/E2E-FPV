...
## 6.30
+ 又出现了测试的时候只向一个方向飞，推测原因：
  1. 输入的past_actions/目标位置淹没了特征向量 →归一化了
  2. 学习率太小 → 从1e-4改到3e-4
  3. （gemini推测）loss量级上daggerloss太大，导致智能体倾向于学习专家数据均值 → 加了0.5的权重

## 7.1 
+ 继续试图解决动作变化幅度太小
  1. 在张量拼接之后加了一个层归一化再送进mlp
  2. daggerloss再次降到0.1
  3. 【怀疑是不是不该输入past actions】还没试验
  4. 把scale改成不对称scale，即0.62-0.64映射到(0,10)而非(-5,5) 
+ 服了 原来是之前另外搞一个test_agent评价结果忘关了 导致每次测试的时候都在用一个初始网络评价
+ 但是改了之后用评价模式飞还是输出差不多的动作
+ 把dagger和评价拆开了，dagger模式网络evaluate=False，观察多训一点是不是评价的时候能灵活一点

## 7.2
+ 总是报段存储错误，把buffer从30000一路降到5000终于没问题了
+ 无人机在第一段飞不好，感觉是dagger权重太低，再增加到1.0

## 7.3
+ 发现程序正常运行了一晚上，5000的buffer size应该是比较内存友好了
+ 无人机还是飞不好第一阶段，把aux loss weight降到0.3试试
+ 研究一下PID，万一总是学不出来就还是用高一点的控制（或者先测试一下略高一点的控制的响应怎么样）
+ 为了加快训练对网络进行轻量化，改成两个残差块+1层GRU，残差块输出特征向量维度降到256
+ 网络轻量化似乎还是效果不大，定量研究影响训练时间的因素：
  + 批量大小：256时，采样时间、Q网络反向传播时间、策略网络传播时间为0.066、0.05、0.2s左右；64时，分别为0.017、0.014、0.056s左右
+ 改了一个纯强化和一个纯dagger，都跑着试试
+ 绷不住了 最后发现是MPC初始化的时候会输出一个巨他妈大的离群值（-30），还占dagger数据的接近1/10，loss能降下来就怪了
  + mpc初始值给到0.64，输出之前clip一下
  + loss好多了，感觉学起来了，但是dagger的动作还是不怎么多元化
+ 为了促进dagger动作多元化，mpc轨迹和dagger轨迹按照1epoch的频率轮流执行；在dagger过程中也进行学习，20轮之后测试

## 7.4
+ 好消息 loss降到0.01量级了 坏消息 还是飞不好
+ 平衡dagger和mpc的次数，dagger的episode直接*5
+ 还是不行...把dagger loss的weight提高到10, aux loss的weight降到0.1
+ 还是不行 一怒之下把mpc轨迹的训练关了 只在dagger过程内训练
+ 好不容易看到效果了 结果ue4崩了 气死我了
+ 把q网络训练的部分关掉，memory不存额外的next img了
  + bestreward从-1000开始，多保存一点模型

## 7.7
+ 把完全没用的past action删了,batch size降到了64

## 7.8
+ batchsize进一步降到32 换了新的weighted MSE
+ 效果不错，能偶尔飞过门了，但是还不稳定

## 7.9
+ 仔细排查了一下，怀疑和图像序列间隔有关系，现在的一个T内四张照片间隔是0.02s，但是两个批次之间间隔是0.04s
  + 改代码，把T内间隔改成0.25s；还给GRU加入了隐藏状态传递机制，通过控制hidden state实现运行和训练解耦
  + 还是不行

## 7.10
+ 加入了一个很小的近期动作buffer，缓解只从老数据取样的问题
  + 顺便改了参数更新逻辑，现在所有buffer的数据拼成一个大batch来训练，不同memory的权重通过数据占总batch比例体现
  + 效果很差，先回退掉合并batch这一步
+ 写了基于物理的损失函数，保留了小批次buffer，先跑跑试试

## 7.11
+ 把辅助头输出都打印了，发现位置和速度都太大了，网络根本拟合不了（1e1和1e2级别），全都除以10试试（同时修改了loss权重）
+ 趁着训练崩溃给物理损失函数也加了基于方差的权重，然后直接复制了gemini的网络初始化代码，最后把batch size升到96

## 7.15
+ 训了960个episode，loss降得很低了，但是还没有满足精确飞行的需求，调整dagger的权重为1.0（增加mpc数据比例），此时模型是master_160_-1021.32_32.7104_25.2_model
+ 试着增大一下MPC对z高度的权重，从50加到100，y方向权重降到0.5，此时模型是master_223_-1213.96_7.8633_33.6_model
+ 只改权重好像不太行，把高度目标改成1.8m，此时模型是master_225_-907.31_11.2273_22.6_model
...
+ Q值带着rl loss一起飞到61亿了...
  ...
  + 奖励总值除以20
  + critic网络最后一层初始化到0附近
  + Q_state归一化（和aux_head一样的系数）

## 8.8
+ 通过拆掉两根16G内存解决了训练崩溃问题
+ 把q_loss和dis系数从取最大值改成了相乘
+ 拿掉了一个门，变成穿过一个门去找目标，改完代码开始从头训练

## 8.10
+ 发现碰撞惩罚过大的时候Q值又会飞，而且学不会，降回去了
+ 降低了buffer_size到1000，略微模仿一下on-policy
+ 还是不行的话就定期clear或者彻底转PPO
  
## 8.12
+ 确认了目前这种训练方式无法实现稳定训练，因此：
  1. 加入模拟崩溃的定期重置机制，先设置每1000次训练后重置权重参数和buffer
  2. 同时先写着PPOfD
+ 先加入了存10个模型就清空buffer并重置权重系数的机制；两权重的处理方式也变回相乘

## 8.14
+ 目前只实现了重置memory和权重，还需要验证最佳模型加载和Adam重置

## 8.15
+ 训练结果证明只靠重置memory和自适应权重还不够，加入最佳模型加载和Adam重置
  + 目前在训练episode数大于100之后，每生成20个新模型后会重新加载最好模型
+ 怀疑碰撞惩罚给得太大，导致智能体远离整个门，顺便把碰撞惩罚从200降到50，centering系数从1.0加到2.0
+ 发现又开始输出均值了...权重更新间隔从500改成100
+ batch从32加到256试试 policy输出层初始化范围从-1~1缩到-0.5~0.5绝望

## 8.16
+ 还是不行 回滚到32的batch 干脆纯模仿试试

## 10.16
+ 用固定数据集纯模仿的方式测试了网络具有减小loss的能力
+ 修改穿门判定，必须从门框中心穿越，并在穿门时刻给予一次奖励
+ 修改了奖励函数，目前的奖励函数：
  + 基于势能（相对当前目标的距离+速度、相对最终目标的距离）
  + 成本函数（控制指令、角速率、耗时尽可能小，朝向对齐门中心）
  + 事件奖励（到达最终目标或碰撞）
  + 穿门奖励：穿门时给出一次
  + **奖励函数需要调参**
+ 注释掉动态混合rl+il机制，暂时改成线性衰减（类似epsilon-greedy）
  + self.baseline_update_window = args['baseline_update_window'] 作为衰减长度
  + self.baseline_update_gamma = args['baseline_update_gamma'] 作为最终il占比
+ 增大辅助头loss权重
+ 目标网络更新权重tau从0.2改到0.01
+ batch_size加到64
+ 增加z值方向补偿变量，弥补UE4中门框中心在底部和实际门框中心的偏差；状态变量中的相对位置现在也相对门框中心了
+ mpc损失函数优化目标增加相机对准目标项，鼓励无人机对准目标飞（修改前姿态要求固定保持水平）
+ 修正了final_target部分相减反了的bug
+ 无人机动作范围从0.62-0.66改到0.55-0.73，加强机动性
+ 修改了step函数的逻辑，在获取状态后重新判定阶段并再次获取状态，解决了穿门时阶段目标更新与状态更新异步的问题

## 10.23
+ 为便利训练对代码进行了一些调整，main.py中每个episode的dagger倍数由5调整到2；开始训练的条件由够一个batch改为5倍batch大小
+ 再次修改了奖励机制和穿门判定机制，只要y坐标超过门就切换目标
  + 奖励函数仅在穿过门时给予一次事件奖励，绕过给予负奖励，保证相机图像相同时对应Q_state和Q值类似
  + 目前的方案下，穿门/绕门对于相同照片对应的Q_state和Q值影响很大
+ rl权重增长周期从20000增加到50000
...
## 12.19 --cz
+ 以穿门为唯一目标，首先进行消融实验，aux_weight=0
+ 橙球缩小以避免干扰（最简化fix）
+ 加速训练：
  + 最大训练轮数1000000→300000
  + warmup:100000→30000
  + 学习率:1e-4→3e-4
  + rl比重上升周期：500000→150000
  + 目标网络更新间隔：20→50

## 12.26 --cz
+ aux weight加到0.5进行正常训练
  
## 12.30 --cz
+ 发现12.19版本和12.26相比，成功率反而不好看
+ 去掉辅助头，轮数、warmup等都恢复到12.19版本，再训练一次试试