...
class env:
    ...
    def get_drone_state(self): # 给MPC的13维向量和给Q网络的25维状态向量
        ...
        return np.concatenate((fpv_pos, fpv_vel, fpv_attitude, fpv_angular_vel)), \
            np.concatenate((fpv_vel/10.0, attitude_6d, fpv_angular_vel, 
                            relative_pos_door_one/10.0, relative_vel_door_one/10.0, relative_pos_target/10.0)), \
    ...
    def reset(self):
        ...
        #                         0.6, 0.6, 0.6, 0.6,
        #                         0.6, 0.6, 0.6, 0.6])
        self.final_pi_target = np.array([self.final_target_state[0], self.final_target_state[1]/33.0, self.final_target_state[2]/(-3.0)]) # 只看位置
    ...
    def step(self, control_signal):
        ...
        # 计算进度奖励 (R_progress)
        R_approach = - np.linalg.norm(Q_state[18:21]) # 距离最终目标的距离
