...
# CEM参数
PREDICTION_HORIZON = 10  # MPC预测长度 (N_steps)
...
mavlink_connection_string = 'udp:127.0.0.1:14550' # 默认的PX4 SITL GCS端口
mavlink_target_system = 0      # 默认的MAVLink无人机目标坐标系
mavlink_target_component = 0   # 默认的MAVLink自动驾驶仪component ID
takeoff_altitude = 3.0         # PX4 MAV_CMD_NAV_TAKEOFF指令高度
control_mode_px4 = "OFFBOARD"  # PX4控制模式("OFFBOARD" or "POSITION")
                                            # OFFBOARD 需要连续输入指令，加速度控制用这个
                                            # POSITION 在输出MAV_CMD_NAV_WAYPOINT一类的指令时比较好用
px4_vehicle_name_in_airsim = "PX4"