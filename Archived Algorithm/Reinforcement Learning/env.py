...
class env:
    ...
    def reset(self):
        ...
        self.info = 0
        self.done = False
        self.past_actions = np.array([0.64, 0.64, 0.64, 0.64,
                                0.64, 0.64, 0.64, 0.64,
                                0.64, 0.64, 0.64, 0.64])
        ...
        self.initial_target_distance = np.linalg.norm(Q_state[24:27]) # 距离最终目标的距离
        self.center_distance = np.linalg.norm(Q_state[12 : 15])
    ...
    def step(self, control_signal):
        ...
        # 27维Q网络状态：0-5:无人机姿态（6D连续表示），6-8:世界系速度，9-11：本体系角速度，12-14：相对第一个门的位置，15-17：相对第一个门的速度
        # 18-20：相对第二个门的位置，21-23：相对第二个门的速度，24-26：相对最终目标的位置
        # 计算进度奖励 (R_progress)
        R_approach = self.initial_target_distance - np.linalg.norm(Q_state[24:27]) # 缩短的距离
        self.initial_target_distance = np.linalg.norm(Q_state[24:27])

        R_velocity_align = 0
        if self.phase_idx != 2:
            R_centering = self.center_distance - np.linalg.norm(Q_state[12 + self.phase_idx * 6 : 15 + self.phase_idx * 6]) # 相对下一个门的距离
            R_velocity_align = - np.linalg.norm(Q_state[15 + self.phase_idx * 6]) # 相对下一个门x方向的速度
            self.center_distance = np.linalg.norm(Q_state[12 + self.phase_idx * 6 : 15 + self.phase_idx * 6])

        R_progress = 1.0 * R_approach + 1.0 * R_centering + 5.0 * R_velocity_align # 进度奖励加权
        ...
        c2 = Q_state[3:6]
        c3 = np.cross(c1, c2)
        c3_normalized = c3/np.linalg.norm(c3) # 如果c3是零向量需要处理异常
        # 定义世界Z轴
        z_world = [0.0, 0.0, - 1.0]
        # 计算点积作为奖励
        R_attitude= - np.dot(c3_normalized, z_world) 

        R_stability_cost = - np.linalg.norm(current_drone_state[9:12]) # 无人机角速度尽可能小
        R_time_cost = - 1.0 # 耗时代价
        R_height = Q_state[14] # 奖励无人机飞得高一点
        R_cost = R_action_cost + R_stability_cost * 10 + R_time_cost + R_attitude * 100 + R_height * 30
        print(f"控制代价:{R_action_cost}, 角速度代价：{R_stability_cost * 10}, 姿态奖励:{R_attitude * 100}, 高度奖励:{R_height * 30}")
        ...
        print(f"进度奖励:{R_progress}, 事件奖励:{R_event}, 代价奖励:{R_cost}")
