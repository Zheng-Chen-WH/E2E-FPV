...
from utils import soft_update, hard_update, six_d_to_rot_mat
...
class SAC(object):
    ...
    def update_parameters(self, rl_memory, batch_size, updates):
        ...
        qf1_pi, qf2_pi = self.critic(rl_q_state_batch, rl_pi)
        min_qf_pi = torch.min(qf1_pi, qf2_pi)

        rl_policy_loss = ((self.alpha * rl_log_pi) - min_qf_pi).mean() # Jπ = ��st∼D,εt∼N[α * logπ(f(εt;st)|st) − Q(st,f(εt;st))] 原论文式(7)
        ...
        total_policy_loss = self.aux_loss_weight * rl_aux_loss + rl_policy_loss
