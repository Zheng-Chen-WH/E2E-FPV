...
dagger_memory = ReplayMemory(args['replay_size']) # DAgger过程的memory
recent_memory = ReplayMemory(args['recent_buffer_size'])
...
if args['task']=='Train':
    ...
    if args['LOAD PARA']==True:
        agent.load_model("master_126_-851.31_17.5411_19.4", evaluate=False)
    ...
    for i_episode in itertools.count(1): #itertools.count(1)用于创建一个无限迭代器。它会生成一个连续的整数序列，从1开始，每次递增1。
        ...
        while episode_steps <= args['max_steps']:
            ...
            if math.fabs(scaled_MPC_action[0]) < 10 and scaled_MPC_action[0] > 0 and not done:
                memory.push(img_tensor, scaled_MPC_action, final_pi_target,
