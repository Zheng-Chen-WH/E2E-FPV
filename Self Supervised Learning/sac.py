...
class SAC(object):
    def __init__(self, pi_num_inputs, Q_num_inputs, action_dim, args):
        ...
        self.critic = QNetwork(Q_num_inputs, action_dim, args['hidden_sizes'], args['activation']).to(self.device)
        self.critic_optim = Adam(self.critic.parameters(), self.base_lr)

        self.critic_target = QNetwork(Q_num_inputs, action_dim, args['hidden_sizes'], args['activation']).to(self.device)
        ...
        if self.automatic_entropy_tuning is True: #原论文直接认为目标熵就是动作空间维度乘积的负值，在这里就是Box的“体积”
            ...
            self.target_entropy = - action_dim # 对于一维动作空间向量，目标值就是这个
        ...
        self.policy = GaussianPolicy(embedding_dim, frames, pi_num_inputs, action_dim, args['hidden_sizes'], args['activation'], args['max_action'], args['min_action']).to(self.device)
