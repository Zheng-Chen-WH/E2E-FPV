...
class ReplayMemory:
    ...
    def push(self, pi_img, pi_state, q_state, action, reward, next_pi_img, next_pi_state,
              next_q_state, done, goal, resnet_position, resnet_attitude,
              gru_velocity, gru_angular):
    ...
    def sample(self, batch_size):
        ...
        pi_img, pi_state, q_state, action, reward, next_pi_img, next_pi_state, next_q_state, done, goal, \
...
class DAggerMemory:
    ...
    def push(self, pi_img, pi_state, action, goal,
              resnet_position, resnet_attitude, gru_velocity, gru_angular):
        if len(self.buffer) < self.capacity:
            self.buffer.append(None) #没有充满时先用none创造出self.position
        self.buffer[self.position] = (pi_img.squeeze(0), pi_state, action, goal,
                                      resnet_position, resnet_attitude, gru_velocity, gru_angular)
        # 把两个视频张量的多余维度挤掉，输出stack的时候才能叠回正常batchsize
        self.position = (self.position + 1) % self.capacity #超出记忆池容量后从第一个开始改写以维持容量不超标
    ...
    def sample(self, batch_size):
        batch = random.sample(self.buffer, batch_size)
        pi_img, pi_state, action, goal, \
            resnet_position, resnet_attitude, gru_velocity, gru_angular = map(np.stack, zip(*batch))
        return pi_img, pi_state, action, goal, \
            resnet_position, resnet_attitude, gru_velocity, gru_angular
