...
class SAC(object):
    ...
    def update_parameters(self, memory, batch_size, updates):
        pi_img_batch, pi_state_batch, q_state_batch, action_batch, reward_batch, next_pi_img_batch, \
            next_pi_state_batch, next_q_state_batch, done_batch, goal_batch,\
                  resnet_position_batch, resnet_attitude_batch, gru_velocity_batch, gru_angular_batch  = memory.sample(batch_size=batch_size)
        ...
        resnet_position_batch = torch.FloatTensor(resnet_position_batch).to(self.device)
        resnet_attitude_batch = torch.FloatTensor(resnet_attitude_batch).to(self.device)
        gru_velocity_batch = torch.FloatTensor(gru_velocity_batch).to(self.device)
        gru_angular_batch = torch.FloatTensor(gru_angular_batch).to(self.device)
        ...
        with torch.no_grad():
            ...
            qf1_next_target, qf2_next_target = self.critic_target(next_q_state_batch, next_state_action) #target算出来的q值
        ...
        qf1, qf2 = self.critic(next_q_state_batch, next_state_action)  # Two Q-functions to mitigate positive bias in the policy improvement step
