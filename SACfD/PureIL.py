...
class SAC(object):
    ...
    # 数据准备阶段 (在CPU上准备好所有数据, 然后一次性转移)
        ...
        rl_q_state_batch = torch.from_numpy(exp_q_state)
        rl_action_batch = torch.from_numpy(exp_action)
        rl_reward_batch = torch.from_numpy(exp_reward).unsqueeze(1)
        rl_next_q_state_batch = torch.from_numpy(exp_next_q_state)
        rl_done_batch = torch.from_numpy(exp_done).unsqueeze(1)
        rl_next_pi_img_batch = torch.from_numpy(exp_next_pi_img)
        rl_next_goal_batch = torch.from_numpy(exp_goal)
        
        # CPU上构建用于Policy更新的统一批次 (dagger + expert + exploration)
        # 把所有用于策略更新的状态和目标都拼接起来，后面用切片的方法再分开
        policy_pi_img_batch = torch.from_numpy(exp_pi_img)
        policy_goal_batch = torch.from_numpy(exp_goal)
        
        # policy_q_state批次包含了策略网络需要评估的所有状态
        policy_q_state_batch = torch.from_numpy(exp_q_state) # Dagger数据没有Q状态

        # 用于计算IL Loss的“真值”动作 (dagger + expert)
        # 注意：探索数据没有对应的专家动作，我们可以用零向量填充，但更简单的做法是在计算loss时只使用相应部分
        il_action_batch = torch.from_numpy(exp_action)
        
        # 用于辅助损失的真值
        policy_res_pos_batch = torch.from_numpy(exp_res_pos)
        policy_res_att_batch = torch.from_numpy(exp_res_att)
        policy_gru_vel_batch = torch.from_numpy(exp_gru_vel)
        policy_gru_ang_batch = torch.from_numpy(exp_gru_ang)
        ...
        il_policy_loss_component = physics_MSE(pi, il_action_batch) + physics_MSE(pi, il_action_batch)
        ...
        return il_policy_loss_component.item(), alpha_loss.item(), alpha_tlogs.item()
